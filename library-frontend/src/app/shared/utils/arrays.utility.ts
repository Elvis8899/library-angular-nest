...
export class ArrayUtility {
   ...
   */
  static BeautifyArrayOfNumbers(input: string | (number | string)[]): number[] {
    let array: (string | number)[];

    if (typeof input === "string") {
      try {
        const parsedString = input.includes("[") ? input : `[${input}]`;
        array = JSON.parse(parsedString);
      } catch (error) {
   ...
   */
  static EnumToStringArray<T extends Record<string, string | number>>(
    enumObj: T,
    keyArray = false
  ): string[] {
    if (keyArray) {
   ...
   */
  static EnumToKeyValueArray<T extends Record<string, string | number>>(
    enumObj: T,
    translateService?: TranslateService,
    keyCaseSpilt = true
  ): { key: string; value: string }[] {
    const translateValue = translateService
      ? translateService.instant
      : (v: string) => v;
    return Object.keys(enumObj)
      .filter((key) => isNaN(Number(key)))
      .map((key) => ({
        key: translateValue(HumanizeSlugUtility.humanize(key, keyCaseSpilt)),
